...
module.exports = (() => {
  ...
  regex.fractions = {};
  // FIXED: Removed the unnecessary backslash before the hyphen
  ...
  function parsePrice(value, unitCode, fractionSeparator, specialFractions, thousandsSeparator) {
    ...
    // FIXED: Removed the self-assignment. 
    // We only change fractionSeparator if the conditions are NOT met.
    if (!(is.string(fractionSeparator) && fractionSeparator.length < 2)) {
      if (unitCodeItem.supportsFractions && regex.fractions.separators.test(value)) {
    ...
    if (unitCodeItem.supportsFractions && fractionSeparator !== '.') {
      ...
      const integerPart = parseInt(integerCharacters, 10);
      const fractionPart = parseInt(fractionCharacters, 10);
