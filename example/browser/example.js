(function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){let c='function'==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);let a=new Error('Cannot find module \''+i+'\'');throw a.code='MODULE_NOT_FOUND',a;}let p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){let n=e[i][1][r];return o(n||r);},p,p.exports,r,e,n,t);}return n[i].exports;}for(let u='function'==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o;}return r;})()({1:[function(require,module,exports){
  ...
  module.exports = (() => {
    ...
    let PageModel = function () {
      let that = this;
      let connection = null;
      let diagnostics = null;
      that.hostname = ko.observable('wsqs-cf.aws.barchart.com');
      let timezoneLocal = timezones.guessTimezone();
      let timezonesList = [];
      ...
      that.canReset = ko.computed(function () {
        let connected = that.connected();
        let activeTemplate = that.activeTemplate();
        ...
        if (data.event) {
          ...
          if (event === 'login success') {
            ...
            if (that.diagnosticsEnabled() && diagnostics === null) {
              ...
              let replayFile = that.replayFile();
              let replaySymbols = that.replaySymbols().toString().split(',');
              let subscriptions = [];
              function bindReplaySymbol(s) {
                let model = new RowModel(s, that.timezone);
                let handleMarketUpdate = function (message) {
              ...
              for (let i = 0; i < replaySymbols.length; i++) {
      ...
      that.connect = function () {
        ...
        let diagnosticsEnabled = that.diagnosticsEnabled();
        let replayFile = that.replayFile();
        let replaySymbols = that.replaySymbols();
      ...
      that.showCumulativeVolume = function (model) {
        ...
        if (!model.getCumulativeVolumeHandler()) {
          ...
          let handleCumulativeVolume = function (message) {
            ...
            if (message.event === 'reset') {
              ...
              let firstPriceLevel = null;
              let priceLevel = ko.utils.arrayFirst(priceLevels(), function (item) {
                if (firstPriceLevel === null) {
                  firstPriceLevel = item;
                }
                return message.price === item.price();
              }) || null;
              let existing = priceLevel !== null;
              if (existing) {
      ...
      that.diagnosticsScroll = function () {
        if (that.diagnosticsEnabled() && diagnostics !== null) {
          let desiredIndex = that.diagnosticsIndex();
          try {
    ...
    let RowModel = function (symbol, timezone) {
      let that = this;
      that.symbol = symbol;
    ...
    let PriceLevelModel = function (price, volume) {
      let that = this;
      ...
      case '%':
      {
        ...
        if (xmlDocument) {
          ...
          case 'QUOTE':
          {
            ...
            // (i.e. "session" letiable) -- instead of taking it from the "combined"
            // session.
  ...
  module.exports = (() => {
    ...
    function checkArgumentType(letiable, letiableName, type, typeDescription, index) {
      if (type === String) {
        if (!is.string(letiable)) {
          throwInvalidTypeError(letiableName, 'string', index);
        }
      } else if (type === Number) {
        if (!is.number(letiable)) {
          throwInvalidTypeError(letiableName, 'number', index);
        }
      } else if (type === Function) {
        if (!is.fn(letiable)) {
          throwInvalidTypeError(letiableName, 'function', index);
        }
      } else if (type === Boolean) {
        if (!is.boolean(letiable)) {
          throwInvalidTypeError(letiableName, 'boolean', index);
        }
      } else if (type === Date) {
        if (!is.date(letiable)) {
          throwInvalidTypeError(letiableName, 'date', index);
        }
      } else if (type === Array) {
        if (!is.array(letiable)) {
          throwInvalidTypeError(letiableName, 'array', index);
        }
      } else if (!(letiable instanceof (type || Object))) {
        throwInvalidTypeError(letiableName, typeDescription, index);
      }
    }
    function throwInvalidTypeError(letiableName, typeDescription, index) {
      let message;
      if (typeof index === 'number') {
        message = `The argument [ ${letiableName || 'unspecified'} ], at index [ ${index.toString()} ] must be a [ ${typeDescription || 'unknown'} ]`;
      } else {
        message = `The argument [ ${letiableName || 'unspecified'} ] must be a [ ${typeDescription || 'Object'} ]`;
      }
      throw new Error(message);
    }
    function throwCustomValidationError(letiableName, predicateDescription) {
      throw new Error(`The argument [ ${letiableName || 'unspecified'} ] failed a validation check [ ${predicateDescription || 'No description available'} ]`);
    }
   ...
   */
     ...
     *
     * @static
     * @param {*} letiable - The value to check.
     * @param {String=} letiableName - The name of the value (used for formatting an error message).
     * @param {*=} type - The expected type of the argument.
     * @param {String=} typeDescription - The description of the expected type (used for formatting an error message).
     */
      argumentIsRequired(letiable, letiableName, type, typeDescription) {
        checkArgumentType(letiable, letiableName, type, typeDescription);
      },
      /**
     ...
     * @param {String=} typeDescription - The description of the expected type (used for formatting an error message).
     */
      argumentIsOptional(letiable, letiableName, type, typeDescription, predicate, predicateDescription) {
        if (letiable === null || letiable === undefined) {
          return;
        }
        checkArgumentType(letiable, letiableName, type, typeDescription);
        if (is.fn(predicate) && !predicate(letiable)) {
          throwCustomValidationError(letiableName, predicateDescription);
        }
      },
      argumentIsArray(letiable, letiableName, itemConstraint, itemConstraintDescription) {
        this.argumentIsRequired(letiable, letiableName, Array);
        if (itemConstraint) {
          let itemValidator;
          if (typeof itemConstraint === 'function' && itemConstraint !== Function) {
            itemValidator = (value, index) => itemConstraint.prototype !== undefined && value instanceof itemConstraint || itemConstraint(value, `${letiableName}[${index}]`);
          } else {
            itemValidator = (value, index) => checkArgumentType(value, letiableName, itemConstraint, itemConstraintDescription, index);
          }
          letiable.forEach((v, i) => {
            itemValidator(v, i);
          });
        }
      },
     ...
     */
      argumentIsValid(letiable, letiableName, predicate, predicateDescription) {
        if (!predicate(letiable)) {
          throwCustomValidationError(letiableName, predicateDescription);
        }
      },
  ...
  module.exports = (() => {
    ...
    /**
   * Utilities for interrogating letiables (e.g. checking data types).
   *
   * @public
   * @module lang/is
   */
 ...
 */
  ...
  let validators = validator.validators;
  /**
  ...
  (function (process){(function (){
    ...
    let defaults = {
      ...
      transformResponse: [function transformResponse(data) {
        let transitional = this.transitional;
        let silentJSONParsing = transitional && transitional.silentJSONParsing;
        let forcedJSONParsing = transitional && transitional.forcedJSONParsing;
        let strictJSONParsing = !silentJSONParsing && this.responseType === 'json';
 ...
 */
  ...
  let currentVerArr = pkg.version.split('.');
 ...
 */
  function isOlderVersion(version, thanVersion) {
    let pkgVersionArr = thanVersion ? thanVersion.split('.') : currentVerArr;
    let destVer = version.split('.');
    for (let i = 0; i < 3; i++) {
 ...
 */
  validators.transitional = function transitional(validator, version, message) {
    let isDeprecated = version && isOlderVersion(version);
 ...
 */
  ...
  let _index = _interopRequireDefault(require('../tzTokenizeDate/index.js'));

  let _index2 = _interopRequireDefault(require('../newDateUTC/index.js'));
  ...
  }; // Parse letious time zone offset formats to an offset in milliseconds

  function tzParseTimezone(timezoneString, date, isUtcDate) {
    let token;
    let absoluteOffset; // Empty string
  ...
  function calcOffset(date, timezoneString) {
    let tokens = (0, _index.default)(date, timezoneString); // ms dropped because it's not provided by tzTokenizeDate

    let asUTC = (0, _index2.default)(tokens[0], tokens[1] - 1, tokens[2], tokens[3] % 24, tokens[4], tokens[5], 0).getTime();
  ...
  function fixOffset(date, offset, timezoneString) {
    let localTS = date.getTime(); // Our UTC time is just a guess because our offset is just a guess

    let utcGuess = localTS - offset; // Test whether the zone matches the offset for this ts

    let o2 = calcOffset(new Date(utcGuess), timezoneString); // If so, offset didn't change, and we're done
 ...
 */
  function tzTokenizeDate(date, timeZone) {
    let dtf = getDateTimeFormat(timeZone);
  ...
  function partsOffset(dtf, date) {
    try {
      let formatted = dtf.formatToParts(date);
      let filled = [];

      for (let i = 0; i < formatted.length; i++) {
        let pos = typeToPos[formatted[i].type];
  ...
  function hackyOffset(dtf, date) {
    ...
    let parsed = /(\d+)\/(\d+)\/(\d+),? (\d+):(\d+):(\d+)/.exec(formatted); // let [, fMonth, fDay, fYear, fHour, fMinute, fSecond] = parsed
  ...
  function getDateTimeFormat(timeZone) {
    ...
    // New browsers use `hourCycle`, IE and Chrome <73 does not support it and uses `hour12`
      let testDateFormatted = new Intl.DateTimeFormat('en-US', {
        hourCycle: 'h23',
      ...
      let hourCycleSupported = testDateFormatted === '06/25/2014, 00:00:00' || testDateFormatted === '‎06‎/‎25‎/‎2014‎ ‎00‎:‎00‎:‎00';
  ...
  let _index = _interopRequireDefault(require('../_lib/tzParseTimezone/index.js'));
